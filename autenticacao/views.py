...
# siginifica que posso verifcar o username se existe
...
def logar(request):
    if request.method == "GET":
        ...
        if not usuario:
            messages.add_message(request, constants.ERROR, 'Algo de errado não está certo !')
            return redirect('/auth/logar')
        else:
            auth.login(request, usuario)
            return redirect('/sinfo/home')
